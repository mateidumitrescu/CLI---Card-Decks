...
void delete_deck(dll_list_t *deck_list) {
	...
	if (current_deck == deck_list->head && deck_list->size == 1) {
		// special case
		free_card_list(&card_list);
		deck_list->size--;
...
void delete_card(dll_list_t *deck_list) {
	...
	if (current_card == card_list->head) {
...
void compare_cards(dll_node_t *current_card, dll_node_t *compare_card) {
	...
	if (info_1->number == info_2->number &&
		strcmp(info_1->symbol, info_2->symbol) == 0) {
		return;
	}
	if (info_1->number > info_2->number) {
		...
		if (strcmp(info_2->symbol, "HEART") == 0 &&
			strcmp(info_1->symbol, "CLUB") == 0) {
			card_t *aux = current_card->data;
			current_card->data = compare_card->data;
			compare_card->data = aux;
		} else if (strcmp(info_2->symbol, "SPADE") == 0 &&
				   strcmp(info_1->symbol, "CLUB") == 0) {
			card_t *aux = current_card->data;
			current_card->data = compare_card->data;
			compare_card->data = aux;
		} else if (strcmp(info_2->symbol, "DIAMOND") == 0 &&
				   strcmp(info_1->symbol, "CLUB") == 0) {
			card_t *aux = current_card->data;
			current_card->data = compare_card->data;
			compare_card->data = aux;
		} else if (strcmp(info_2->symbol, "HEART") == 0 &&
				   strcmp(info_1->symbol, "DIAMOND") == 0) {
			card_t *aux = current_card->data;
			current_card->data = compare_card->data;
			compare_card->data = aux;
		} else if (strcmp(info_2->symbol, "SPADE") == 0 &&
